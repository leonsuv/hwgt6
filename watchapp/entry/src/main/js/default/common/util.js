...
function temp(celsius, metric) {
  ...
  if (metric === false) {
    return decimal(celsius * 9 / 5 + 32, 0) + '\u00b0F';
  }
  return decimal(celsius, 1) + '\u00b0C';
}
...
function buildView(data, opts) {
  ...
  if (view.charging) {
    ...
    view.statusLine = parts.length ? parts.join('  \u00b7  ') : 'laedt';
    view.statusIcon = '\u26a1';
  } else if (view.plugged) {
    view.statusLine = 'Stecker verbunden';
    view.statusIcon = '\u26a1';
  } else if (data.et === 'fuel') {
    view.statusLine = 'Tankfuellung';
    view.statusIcon = '';
  } else {
  ...
  view.climateText = view.climateOn ? 'laeuft' : 'aus';
